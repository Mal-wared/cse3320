...
void bubble_sort_asc_size(long int sizes[], char directory_entries[][256], int n)
{
  for (int i = 0; i < n - 1; i++)
  {
    for (int j = 0; j < n - 1; j++)
...
void bubble_sort_desc_size(long int sizes[], char directory_entries[][256], int n)
{
  for (int i = 0; i < n - 1; i++)
  {
    for (int j = 0; j < n - 1; j++)
...
int main()
{
  ...
  int directory_entry_dates[1024];
  ...
  while (strcmp(command, "Q") != 0)
  {
    switch (command[0])
    {
      case 'D':
      {
        ...
        {
          bubble_sort_desc_size(de_first_sizes, de_first_five, de_max);
        }
        if (strcmp(sort_mode, "SS") == 0)
        {
          bubble_sort_asc_size(de_first_sizes, de_first_five, de_max);
        }
        ...
        for (int i = 0; i < de_max; i++) 
        {
          if (i != 0)
          {
            printw("%d. %ld bytes\t %s", current_de_count + i, de_first_sizes[i], de_first_five[i]);
            file_place++;
            move(file_place, 15);
          }
          else
      ...
      case 'C': 
      {
        ...
        {
          strcpy(destination, "/media/sf_GitHub_Repos/cse3320/EX_DIR1");
          strcpy(directory_to_enter, destination);
        }
